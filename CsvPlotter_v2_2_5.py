...
class FileProcessor:
    ...
    def setPath(self, input_paths):
        ...
        if len(self.file_paths) == 1 and os.path.isdir(self.file_paths[0]):
            ...
            self.file_paths = [os.path.join(directory, f) for f in os.listdir(directory) if f.endswith(('.csv', '.txt'))]
    ...
    def _detect_metadata_and_header(self, lines):
        count_numeric = 0
        countmax_numeric = 3
        metadata_end_row = -1
        header_row = -1
        for i, line in enumerate(lines):
            values = [v.rstrip() for v in line.strip().split(',')]
            if any(v.isalpha() for v in values):
                header_row = i
            if all(self._is_numeric(v) or v == '' for v in values):
                count_numeric += 1
    ...
    def _detect_empty_columns(self, lines, metadata_end_row):
        count_empty_col = 0
    ...
    def load_csvs(self):
        try:
            for self.file_path in self.file_paths:
                ...
                metadata_end_row, header_row = self._detect_metadata_and_header(lines)
                empty_col = self._detect_empty_columns(lines, metadata_end_row)
                ...
                df = pd.read_csv(self.file_path, header=header_row if header_row >= 0 else None)
                if empty_col > 0:
                    df = df.drop(df.columns[range(empty_col)], axis=1)
                if header_row < 0 or all(self._is_numeric(col) for col in df.columns):
            ...
            for col in range(self.screen_columns):
                for filename in self.processed_data_matrix[row][col].keys():
                    ...
                    for colname in df.columns:
                        self.processed_data_matrix[row][col][filename]['original_y'][labels[i]] = df[colname].to_numpy()
                        i+=1
...
#        y = self.processed_data_matrix[row][col][filename]['lines'][label][0].get_ydata()
    ...
    def setYOffset(self, row, col, filename, label, offset):
        ...
        original_y = self.getOriginalY(row, col, filename, label)
        self.processed_data_matrix[row][col][filename]['lines'][label][0].set_ydata(original_y + offset)
        self.updateDraw()
...
#                self.check_buttons = CheckButtons(self.check_buttons_ax, labels, visibility)
    ...
    def apply_filter(self, ydata, dt, method, cutoff):
        if method == 'N/A' or cutoff <= 0:
            return ydata

        alpha = 2 * np.pi * cutoff * dt
        filtered = np.zeros_like(ydata)
        filtered[0] = ydata[0]
        if method == '1st order LPF':
            for i in range(1, len(ydata)):
                filtered[i] = filtered[i-1] + alpha * (ydata[i] - filtered[i-1])
        elif method == '1st order HPF':
            for i in range(1, len(ydata)):
                filtered[i] = alpha * (filtered[i-1] + ydata[i] - ydata[i-1])
        return filtered
            ...
            for j in range(self.screen_columns):
                ...
                for filename in self.plotter.getFilenames(i,j):
                    for label in self.plotter.getLabels(i,j,filename).values():
                        selection = self.var_combo_Filter[i][j][k].get()
                        print(selection)
                        k+=1
    ...
    def _update_xoffset(self, event=None):
        for i in range(self.screen_rows):
            for j in range(self.screen_columns):
                ...
                for filename in self.plotter.getFilenames(i,j):
                    for label in self.plotter.getLabels(i,j,filename).values():
                        value_offset = self._get_corrected_float(self.var_entry_XOffset[i][j][k].get())
                        if value_offset != self.plotter.getXOffset(i,j,filename,label):
                            self.plotter.setXOffset(i,j,filename,label,value_offset)
    ...
    def _update_yoffset(self, event=None):
        for i in range(self.screen_rows):
            for j in range(self.screen_columns):
                ...
                for filename in self.plotter.getFilenames(i,j):
                    for label in self.plotter.getLabels(i,j,filename).values():
                        value_offset = self._get_corrected_float(self.var_entry_YOffset[i][j][k].get())
                        if value_offset != self.plotter.getYOffset(i,j,filename,label):
                            self.plotter.setYOffset(i,j,filename,label,value_offset)
...
#                        self.check_Legend[i][j][k] = tk.Checkbutton(self.frameLegend[i][j], text=label, variable=self.var_check_Legend[i][j][k], onvalue=True, offvalue=False, command=self._check_Legend)
                        ...
                        self.var_entry_YOffset[i][j][k] = tk.StringVar(master=self.frameYOffset[i][j], value=f'{xoffset}')
