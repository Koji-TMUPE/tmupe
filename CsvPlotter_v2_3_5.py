...
#                        print(f'visible plotter: {self.plotter.getVisibility(i,j,filename,label)}')
    ...
    def _is_valid_float(self, value: str) -> bool:
        # Defensive: accept non-str inputs without crashing.
        if value is None:
            return False
    ...
    def _get_corrected_float(self, s):
        # Defensive: accept non-str inputs (e.g., float) without crashing.
        if s is None:
            return None
...
#                self.frameOpt[i][j] = tk.Frame(self.frameOpts.scrollable_frame, bg='white', highlightbackground='#BBBBBB', highlightthickness=10)
        ...
        for j in range(col):
            ...
            self.entry_Xmin[j] = tk.Entry(self.frameXaxis)
            self.entry_Xmax[j] = tk.Entry(self.frameXaxis)
...
#                self.frameLegend[i][j] = tk.Frame(self.frameOpt[i][j])
        ...
        for i in range(row):
            for j in range(col):
                ...
                self.entry_Ymin[i][j] = tk.Entry(self.frameYlim[i][j], width=self._width_yentry)
                self.label_Ymax[i][j] = tk.Label(self.frameYlim[i][j], text=f'y max')#({i})({j}):')
                self.entry_Ymax[i][j] = tk.Entry(self.frameYlim[i][j], width=self._width_yentry)
...
#                        self.check_Legend[i][j][k] = tk.Checkbutton(self.frameLegend[i][j], text=label, variable=self.var_check_Legend[i][j][k], onvalue=True, offvalue=False, command=self._check_Legend)
                        ...
                        self.entry_XOffset[i][j][k] = tk.Entry(self.frameXOffset[i][j], width=self._width_xoffsetentry, textvariable=self.var_entry_XOffset[i][j][k])
                        ...
                        self.entry_YOffset[i][j][k] = tk.Entry(self.frameYOffset[i][j], width=self._width_yoffsetentry, textvariable=self.var_entry_YOffset[i][j][k])
                        ...
                        self.entry_Cutoff[i][j][k] = tk.Entry(self.frameCutoff[i][j], width=self._width_cutoffentry, textvariable=self.var_entry_Cutoff[i][j][k])
